...
from dqn.utils import tf_disable_warnings, tf_disable_deprecation_warnings
...
def main():
    ...
    for env_name, events_by_seed in events_by_env_name_by_seed.items():
        plot_env(env_name, events_by_seed, config)
...
def plot_env(env_id, events_by_seed, config):
    ...
    n = min([len(l) for l in xs_list])
    xs_list = [l[:n] for l in xs_list]
    ys_list = [l[:n] for l in ys_list]
